# Hi there, I'm Nasir Hussain 👋

### 👨‍💻 Full Stack Developer & Backend Specialist
Building enterprise-grade systems with a focus on performance, scalability, and clean architecture.

---

### 🚀 Currently Working On:
- **Enterprise Stock Management Systems:** Integrating Node.js with complex Firebird SQL databases and Prisma ORM.
- **Cardiac Output Backend:** Developing high-performance healthcare data processing tools.

### 🛠️ Tech Stack & Tools:
- **Languages:** JavaScript (ES6+), TypeScript
- **Backend:** Node.js, Express, NestJS
- **Database:** Prisma ORM, Firebird SQL, PostgreSQL, MongoDB
- **Tools:** Git, GitHub Actions, Docker, Trello (Agile/Scrum)

---

### 📈 GitHub Stats:
![Nasir's GitHub stats](https://github-readme-stats.vercel.app/api?username=Nasirhussain568&show_icons=true&theme=dark)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Nasirhussain568&layout=compact&theme=dark)

---

### 📫 Connect with me:
- [LinkedIn](https://www.linkedin.com/in/nasir-hussain-4254832a8/))
- [Portfolio](https://YOUR_PORTFOLIO_LINK_HERE)

*"Clean code always looks like it was written by someone who cares."*
